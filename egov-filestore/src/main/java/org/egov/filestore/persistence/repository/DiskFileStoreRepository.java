...
public class DiskFileStoreRepository implements CloudFilesManager {
  private static final Logger log = LoggerFactory.getLogger(DiskFileStoreRepository.class);
  ...
  public DiskFileStoreRepository(FileRepository fileRepository, @Value("${file.storage.mount.path}") String fileMountPath) {
    ...
    if (null == fileMountPath || fileMountPath.isEmpty())
      fileMountPath = FileUtils.getUserDirectoryPath(); 
    this.fileMountPath = fileMountPath;
  }
  
  public void saveFiles(List<Artifact> artifacts) {
    List<Artifact> persistList = new ArrayList<>();
  ...
  public Map<String, String> getFiles(Map<String, String> mapOfIdAndFilePath) {
    ...
    for (String s : mapOfIdAndFilePath.keySet()) {
      StringBuilder url = new StringBuilder(this.hostUrl);
      url.append(this.hostEndpoint);
      url.append(s);
  ...
  public Resource read(FileLocation fileLocation) {
    ...
    if (fileLocation.getFileSource() == null || fileLocation.getFileSource().equals(this.diskFileStorage)) {
      Path path = getPath(fileLocation);
      resource = this.fileRepository.read(path);
      if (resource == null)
        resource = this.fileRepository.read(getPathOldVersion(fileLocation)); 
    } 
    return resource;
  }
  
  private Path getPath(FileLocation fileLocation) {
    return Paths.get(this.fileMountPath, new String[] { fileLocation.getFileName() });
  }
  
  private Path getPathOldVersion(FileLocation fileLocation) {
    return Paths.get(this.fileMountPath, new String[] { fileLocation.getTenantId(), fileLocation.getModule(), fileLocation
          .getFileStoreId() });
  }
