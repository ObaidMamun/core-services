...
public class StorageService {
  ...
  public List<String> save(List<MultipartFile> filesToStore, String module, String tag, String tenantId, String inputStreamAsString) {
    ...
    log.info("Received upload request for jurisdiction: %s, module: %s, tag: %s with file count: %s", new Object[] { module, tag, Integer.valueOf(filesToStore.size()) });
  ...
  private List<Artifact> mapFilesToArtifacts(List<MultipartFile> files, String module, String tag, String tenantId, String inputStreamAsString) {
    ...
    return (List<Artifact>)files.stream().map(file -> {
  ...
  public Resource retrieve(String fileStoreId, String tenantId) throws IOException {
    return this.artifactRepository.find(fileStoreId, tenantId);
  }
  
  public List<FileInfo> retrieveByTag(String tag, String tenantId) {
    return this.artifactRepository.findByTag(tag, tenantId);
  }
  
  public Map<String, String> getUrls(String tenantId, List<String> fileStoreIds) {
    Map<String, String> urlMap = getUrlMap(this.artifactRepository
        .getByTenantIdAndFileStoreIdList(tenantId, fileStoreIds));
    if (this.isNfsStorageEnabled.booleanValue())
      for (String s : urlMap.keySet())
        urlMap.put(s, ((String)urlMap.get(s)).concat("&tenantId=").concat(tenantId));  
    return urlMap;
  }
  
  private Map<String, String> getUrlMap(List<Artifact> artifactList) {
    String src = null;
    if (this.isAzureStorageEnabled.booleanValue())
      src = this.azureBlobSource; 
    if (this.isS3Enabled.booleanValue())
      src = this.awsS3Source; 
    if (this.isNfsStorageEnabled.booleanValue())
      src = this.diskStorage; 
    String source = src;
    Map<String, String> mapOfIdAndFile = (Map<String, String>)artifactList.stream().filter(a -> (null != a.getFileSource() && a.getFileSource().equals(source))).collect(Collectors.toMap(Artifact::getFileStoreId, Artifact::getFileName));
    return this.cloudFilesManager.getFiles(mapOfIdAndFile);
  }
  
  private String getFolderName(String module, String tenantId, Calendar calendar) {
    return tenantId + "/" + module + "/" + calendar.getDisplayName(2, 2, Locale.ENGLISH) + "/" + calendar
      .get(5) + "/";
  }
  
  private String getBucketName(String tenantId, Calendar calendar) {
    if (this.isBucketFixed.booleanValue())
      return this.fixedBucketName; 
    return tenantId.split("\\.")[0] + calendar.get(1);
  }
}
